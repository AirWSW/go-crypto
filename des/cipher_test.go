...
var tripleDESCipherTests = []struct {
	key []byte
	dec []byte
	enc []byte
}{
...
func Test_desCipher_Encrypt(t *testing.T) {
	for i, tt := range desCipherTests {
		c, _ := NewCipher(tt.key)
		got := make([]byte, len(tt.enc))
		c.Encrypt(got, tt.dec)
		if !bytes.Equal(got, tt.enc) {
			t.Errorf("#%d: desCipher.Encrypt() = %016x, want %016x", i, got, tt.enc)
		}
	}
}

func Test_desCipher_Decrypt(t *testing.T) {
	for i, tt := range desCipherTests {
		c, _ := NewCipher(tt.key)
		got := make([]byte, len(tt.dec))
		c.Decrypt(got, tt.enc)
		if !bytes.Equal(got, tt.dec) {
			t.Errorf("#%d: desCipher.Encrypt() = %016x, want %016x", i, got, tt.dec)
		}
	}
}

func Test_tripleDESCipher_Encrypt(t *testing.T) {
	for i, tt := range tripleDESCipherTests {
		c, _ := NewTripleDESCipher(tt.key)
		got := make([]byte, len(tt.enc))
		c.Encrypt(got, tt.dec)
		if !bytes.Equal(got, tt.enc) {
			t.Errorf("#%d: tripleDESCipher.Encrypt() = %016x, want %016x", i, got, tt.enc)
		}
	}
}

func Test_tripleDESCipher_Decrypt(t *testing.T) {
	for i, tt := range tripleDESCipherTests {
		c, _ := NewTripleDESCipher(tt.key)
		got := make([]byte, len(tt.dec))
		c.Decrypt(got, tt.enc)
		if !bytes.Equal(got, tt.dec) {
			t.Errorf("#%d: tripleDESCipher.Decrypt() = %016x, want %016x", i, got, tt.dec)
		}
	}
}
